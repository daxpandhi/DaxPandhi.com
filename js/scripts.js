jQuery(function ($) {
	...
	function CustomFunction() {
		$('.auto').each(function (i, element) {
			...
			var project = $("#main-content").data("project");
			for (let index = 1; index < count; index++) {
				var indexString = index.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false });
				var itemString = project + '/' + prefix + '_' + indexString;
				$(element).append('<div class="collage-thumb"><a class="image-link" href="images/projects/' + itemString + '.webp"><img src="images/projects/' + itemString + '.webp" alt="img" /></a></div>');
			}
		});
	}// End CustomFunction		
	/*--------------------------------------------------
	...
	function PageLoad() {
		...
		if (!$('body').hasClass("disable-ajaxload")) {
			...
			var width = 0,
				...
				time = Math.min(((EstimatedTime / 100) % 500) * 10, 500);
			// Loadbar Animation
			$(".loadbar").animate({
				width: width + "%"
			}, time);
			...
			if (!isMobile()) {
				$(".item-parallax").mouseenter(function (e) {
					...
					if ($this.hasClass("coming-soon")) {
						$("#ball").addClass("with-icon").append('<i class="fa fa-solid fa-ban"></i>');
					} else {
						$("#ball").addClass("with-icon").append('<i class="arrow-icon"></i>');
					}
					$this.closest('.item').find('video').each(function () {
